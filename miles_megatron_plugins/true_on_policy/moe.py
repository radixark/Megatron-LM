...
class SGLangGroupedMLP(GroupedMLP):
    ...
    def _ordered_topk_ids_from_probs(routing_probs: torch.Tensor, topk: int) -> torch.Tensor:
        ...
        expert_ids = torch.arange(num_experts, device=routing_probs.device, dtype=torch.float32)
        probs_fp32 = routing_probs.float()
        tie_step = torch.finfo(torch.float32).eps * probs_fp32.abs().clamp_min(
            1.0 / num_experts
        )
