...
def test_sglang_spec_provider_grouped_mlp_fallback():
    ...
    builder = provider.grouped_mlp_modules(moe_use_grouped_gemm=False)
    assert builder.func.__name__ == "SequentialMLP"
    submodules = builder.keywords["submodules"]
    assert submodules.linear_fc1 is SGLangColumnParallelLinear
    assert submodules.linear_fc2 is SGLangRowParallelLinear

    # Grouped GEMM is not provided by the deterministic SGLang backend; it falls
    # back to the same sequential expert surface (dev removed the pure GroupedMLP).
    grouped_builder = provider.grouped_mlp_modules(moe_use_grouped_gemm=True)
    assert grouped_builder.func.__name__ == "SequentialMLP"
    grouped_submodules = grouped_builder.keywords["submodules"]
    assert grouped_submodules.linear_fc1 is SGLangColumnParallelLinear
    assert grouped_submodules.linear_fc2 is SGLangRowParallelLinear
...
def test_sglang_mtp_spec_uses_sglang_backend():
    ...
    mtp_layer_spec = get_mtp_layer_spec_for_backend(
        mtp_model_layer_spec=transformer_layer_spec, backend=SGLangSpecProvider()
    )
