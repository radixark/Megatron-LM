...
* fused: the production env/config validator and per-weight STE adapter, including
  PyTorch FP32 per-tensor amax + register-resident CuTe DSL QDQ.
...
from megatron.core.extensions.transformer_engine_nvfp4_fake_qat import (
    NVFP4_FAKE_QAT_FLAG,
...
def _benchmark_case(
    ...
    os.environ["NVTE_NVFP4_4OVER6_ERR_USE_FAST_MATH"] = "1" if config.error_use_fp16 else "0"
    ...
    weight_tensors = [
        torch.nn.Parameter(torch.randn((rows, block_axis), dtype=dtype, device="cuda"))
    ...
    qat_config = SimpleNamespace(
        gradient_accumulation_fusion=False, moe_single_grouped_weight=False
    )
    ...
    def fused_end_to_end() -> list[torch.Tensor]:
        return maybe_fake_quantize_nvfp4_weight_tensors(qat_config, False, weight_tensors)

    def fused_precomputed_amax() -> list[torch.Tensor]:
        return [fused_nvfp4_qdq(w, amax, config) for w, amax in zip(weight_tensors, amax_tensors)]
    ...
    for weight_idx, (expected_weight, actual_weight) in enumerate(zip(expected, actual)):
        if not torch.equal(
            ...
            raise AssertionError(
                f"fused QDQ weight {weight_idx} differs from TE in " f"{mismatch_count} elements"
            )
    del expected, actual
...
def main() -> None:
    ...
    for dtype, speedups in speedups_by_dtype.items():
        print(
            f"geomean_speedup_{str(dtype).removeprefix('torch.')}="
            f"{geometric_mean(speedups):.3f}x"
        )
