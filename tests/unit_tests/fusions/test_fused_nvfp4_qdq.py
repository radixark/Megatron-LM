...
def test_te_grouped_linear_real_discrete_weight_qdq_and_backward(
    ...
    expected_weights = [
        _te_reference(weight, layer._nvfp4_qat_config)[0] for weight in weights
    ]
    ...
    assert all(
        actual.main_grad is main_grad
        for actual, main_grad in zip(actual_weights, main_grads)
    )
