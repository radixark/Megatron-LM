...
from megatron.core.utils import is_te_min_version  # noqa: E402
...
pytestmark = [
    ...
    pytest.mark.skipif(
        not is_te_min_version("2.17.0"),
        reason="Fused NVFP4 fake QAT requires Transformer Engine >= 2.17.0",
    ),
...
CONFIGS = [pytest.param(NVFP4QDQConfig(), id="nvfp4")]
...
def _make_input(shape: tuple[int, int], dtype: torch.dtype, init_data: str) -> torch.Tensor:
...
def test_fused_nvfp4_qdq_is_bit_exact_with_te(
    ...
    monkeypatch.setenv("NVTE_NVFP4_4OVER6_ERR_USE_FAST_MATH", "1" if config.error_use_fp16 else "0")
...
@pytest.mark.parametrize(
    ("error_mode", "error_use_fp16"), [("MAE", False), ("MAE", True), ("MSE", False), ("MSE", True)]
)
def test_current_nvfp4_qdq_config_maps_full_latest_te_env_contract(
    ...
    monkeypatch.setenv("NVTE_NVFP4_4OVER6_ERR_USE_FAST_MATH", "1" if error_use_fp16 else "0")
...
def test_te_grouped_linear_real_discrete_weight_qdq_and_backward(
    ...
    expected_weights = [_te_reference(weight, qdq_config)[0] for weight in weights]
    ...
    m_splits = [2, 1, 3]
    inp = torch.ones(
        (sum(m_splits), columns), dtype=torch.bfloat16, device="cuda", requires_grad=True
    )
    ...
    for weight, m_split in zip(weights, m_splits):
        assert weight.grad is not None
        torch.testing.assert_close(
            weight.grad, torch.full_like(weight.grad, m_split), rtol=0, atol=0
        )
