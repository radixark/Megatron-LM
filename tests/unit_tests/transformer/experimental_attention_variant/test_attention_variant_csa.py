...
class TestCompressedSparseAttentionThd:
    ...
    def test_thd_fused_no_grad_and_grad_forwards_share_key_order(self, monkeypatch):
        ...
        # The fused indexer-loss teacher runs whenever sparse loss is on, and its
        # kernels need ``topk`` to be a multiple of 64; this test only looks at
        # the attention ids, so keep every loss path off.
        overrides = {
