...
def current_nvfp4_qdq_config() -> NVFP4QDQConfig:
    ...
    return NVFP4QDQConfig(
        ...
        error_use_fp16=(use_4over6 and _env_flag("NVTE_NVFP4_4OVER6_ERR_USE_FAST_MATH")),
    )
...
@dsl_user_op
def _store_v4_u32(
    base_ptr: Int64, v0: Uint32, v1: Uint32, v2: Uint32, v3: Uint32, *, loc=None, ip=None
) -> None:
    llvm.inline_asm(
        None,
        [
...
class _NVFP4QDQKernel:
    ...
    def kernel(
        ...
        while block < total_blocks:
            ...
            _dequantize_store(
                output_tensor, offset, lo, hi, scale, amax, self.config.e4m3_max, self.is_bfloat16
            )
            block = block + stride
...
def fused_nvfp4_qdq(
    x: torch.Tensor, amax: torch.Tensor, config: Optional[NVFP4QDQConfig] = None
) -> torch.Tensor:
...
class _FusedNVFP4QDQSTE(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx: Any, x: torch.Tensor, amax: torch.Tensor, config: NVFP4QDQConfig
    ) -> torch.Tensor:
