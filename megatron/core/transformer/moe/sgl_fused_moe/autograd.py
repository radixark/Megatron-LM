...
class _FusedExpertsTritonBackward(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx, hidden_states, w1, w2, topk_weights, topk_ids,
        activation, layer_id, num_experts, num_local_experts,
        ep_rank, ep_size, ep_group, allreduce_grad_hidden,
    ):
        with torch.no_grad():
            output = sglang_moe_forward(
                ...
                activation=activation, layer_id=layer_id,
            )
    ...
    def backward(ctx, grad_output):
        ...
        for chunk_start in range(0, num_tokens, chunk_size):
            ...
            invoke_fused_moe_kernel(
                curr_hidden, w1, None, intermediate_cache1, None, None, None,
            ...
            grad_inter3 = curr_grad_output.unsqueeze(1).expand(
            ...
            invoke_fused_moe_backward_kernel(
                grad_output=grad_inter3, input=intermediate_cache2, weight=w2,
                grad_input=grad_inter2, grad_weight=curr_grad_w2,
                grad_topk_weights=curr_grad_topk_weights,
                topk_weights=curr_topk_weights, topk_ids=curr_topk_ids,
                sorted_token_ids=sorted_down, expert_ids=experts_down,
            ...
            invoke_fused_moe_backward_kernel(
                grad_output=grad_inter1, input=curr_hidden, weight=w1,
                grad_input=curr_grad_hidden, grad_weight=curr_grad_w1,
        ...
        if ep_group is not None:
            ...
            if ws > 1:
                ...
                elif os.environ.get("MILES_TRUE_ON_POLICY_MOE_SCALE_HIDDEN_DGRAD_BY_EP", "0") == "1":
                    grad_hidden_states.div_(ws)

        return (
            grad_hidden_states, grad_w1, grad_w2, grad_topk_weights,
            None, None, None, None, None, None, None, None, None,
        )


def sglang_fused_experts_autograd(
    *, layer_number, hidden_states, w1, w2, topk_weights, topk_ids,
    num_experts, num_local_experts, ep_rank, ep_size, ep_group,
    activation="silu", allreduce_grad_hidden=True,
):
    ...
    return _FusedExpertsTritonBackward.apply(
        hidden_states.contiguous(), w1.contiguous(), w2.contiguous(),
        topk_weights.to(dtype=hidden_states.dtype).contiguous(),
        topk_ids.contiguous(),
        activation, layer_number, num_experts, num_local_experts,
        ep_rank, ep_size, ep_group, allreduce_grad_hidden,
    )
