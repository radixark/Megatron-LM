...
def _sglang_ordered_moe_unpermute(
...
class MoEAlltoAllTokenDispatcher(MoETokenDispatcher):
    ...
    def combine_postprocess(self, permutated_local_input_tokens):
        ...
        if self._should_use_sglang_ordered_combine():
            output = _sglang_ordered_moe_unpermute(
    ...
    def _should_use_sglang_ordered_combine(self) -> bool:
        if (
            torch.is_grad_enabled()
            or self.config.moe_router_topk <= 1
            or self.drop_and_pad
            or self.config.moe_permute_fusion
        ):
            return False
